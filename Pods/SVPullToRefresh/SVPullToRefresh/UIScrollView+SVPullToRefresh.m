...
////
//        NSLog(@"scrollViewDidScroll: contentOffset.y = [%f] self.originalTopInset = [%f]", self.scrollView.contentOffset.y, self.originalTopInset);
//        NSLog(@"scrollViewDidScroll: self.state = [%lu]", (unsigned long)self.state);
//        NSLog(@"scrollViewDidScroll: self.frame = [%@]", NSStringFromCGRect(self.frame));
    if (self.state != SVPullToRefreshStateStopped) {
        CGRect frame = self.frame;
        frame.origin.y = self.scrollView.contentOffset.y + self.originalTopInset + 1;
...
- (void)stopAnimating {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        CGRect frame = self.frame;
        frame.origin.y     = -SVPullToRefreshViewHeight;
